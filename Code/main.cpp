...
int main(int argc, char *argv[]) {
	...
	while(true) {
	    ...
	    UpdateNode(Network[Index], Time, TotalRate, DeficitsValues); //perform rate and increase time
	    UpdateLocalFrailty(Network, Network[Index]); //update the local frailty after the node is modified
	    CalculateRates(Network, Network[Index], tree, Index, TotalRate); //calculate new rates for the node and its neighbours
